...
1. To download a zip file containing the library application, paste the following URL into your browser's address bar: [GitHub Copilot lab - develop unit tests (Java)](https://github.com/MicrosoftLearning/mslearn-github-copilot-dev/raw/refs/heads/main/DownloadableCodeProjects/Downloads/AZ2007LabAppM4Java.zip)

    The zip file is named **AZ2007LabAppM4Java.zip**.

1. Extract the files from the **AZ2007LabAppM4Java.zip** file.

    For example:

    1. Navigate to the downloads folder in your lab environment.
    1. Right-click **AZ2007LabAppM4Java.zip**, and then select **Extract all**.
...
1. Enter the following prompt to understand the existing testing approach:
    ...
    ```plaintext
    #codebase Explain the approach to unit testing that's been implemented in this Java workspace. How are test factories used? What mocking framework is used and why?
    ```
...
1. Enter the following follow-up prompt:
    ...
    ```plaintext
    #codebase What are the benefits of using factory classes like LoanFactory and PatronFactory in unit tests instead of creating objects inline in each test method?
    ```
...
1. Enter the following prompt to understand how to extend testing to the repository layer:
    ...
    ```plaintext
    #codebase How can I extend the test suite to begin testing methods in the JsonLoanRepository class? What's the main difference between testing a service class (like LoanService) and testing a repository class (like JsonLoanRepository)?
    ```
...
1. Enter the following prompt to investigate any failures:
    ...
    ```plaintext
    #codebase The test getLoan_returnsLoanWhenIdFound is failing with: <paste your error here>. What might be wrong with the JsonDataStore path or the loan ID used in the test?
    ```
...
- The C# source for this module is in `az-2007-m4-develop-unit-tests/AccelerateDevGHCopilot`.
- The Java starter for this module is in `az-2007-m4-develop-unit-tests-java/AccelerateDevGHCopilot`.
- `GetLoanTest.java` starts as an empty class that students populate with Copilot.
- `LoanServiceTest.java` and `PatronServiceTest.java` are already fully implemented and serve as the pattern reference.
- The Java equivalent of NSubstitute (C#) is Mockito, already configured in `pom.xml`.
