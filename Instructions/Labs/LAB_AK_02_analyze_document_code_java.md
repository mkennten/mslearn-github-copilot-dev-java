---
lab:
  title: Exercise - Analyze and document code using GitHub Copilot (Java)
  description: Learn how to analyze new or unfamiliar Java code and how to generate documentation using GitHub Copilot in Visual Studio Code.
  duration: 20 minutes
  level: 200
  islab: true
  primarytopics:
...
# Analyze and document code using GitHub Copilot (Java)

GitHub Copilot can help you understand and document a codebase by generating explanations and documentation. In this exercise, you use GitHub Copilot to analyze a Java codebase and generate documentation for the project.

> C# variant: For .NET 9 + xUnit instructions, use `LAB_AK_02_analyze_document_code.md`.
...
Your lab environment must include the following: Git 2.48 or later, Java 21 LTS, Maven 3.9 or later, Visual Studio Code with the Extension Pack for Java and Maven for Java extensions, and access to a GitHub account with GitHub Copilot enabled.

If you're using a local PC as a lab environment for this exercise:

- For help enabling your GitHub Copilot subscription in Visual Studio Code, open the following link in a browser: <a href="https://go.microsoft.com/fwlink/?linkid=2320158" target="_blank">Enable GitHub Copilot within Visual Studio Code</a>.

- Confirm your Java toolchain by running the following commands in a terminal:
...
1. To download a zip file containing the library application, paste the following URL into your browser's address bar: [GitHub Copilot lab - Analyze and document code (Java)](https://github.com/MicrosoftLearning/mslearn-github-copilot-dev/raw/refs/heads/main/DownloadableCodeProjects/Downloads/AZ2007LabAppM2Java.zip)

    The zip file named AZ2007LabAppM2Java.zip will be downloaded to your lab environment.

1. Extract the files from the **AZ2007LabAppM2Java.zip** file.

    For example:

    1. Navigate to the downloads folder in your lab environment.

    1. Right-click **AZ2007LabAppM2Java.zip**, and then select **Extract all**.

    1. Select **Show extracted files when complete**, and then select **Extract**.

1. Open the extracted files folder, then copy the **AccelerateDevGHCopilot** folder to a location that's easy to access, such as your Desktop folder.
...
1. Open the **AccelerateDevGHCopilot** folder in Visual Studio Code.
    ...
    1. Navigate to the Desktop folder, select **AccelerateDevGHCopilot** and then select **Select Folder**.
...
1. Ensure that the project builds and the tests pass successfully.
    ...
    You may see some Warnings, but there shouldn't be any Errors reported, and all tests should pass.
...
1. Ensure that the AccelerateDevGHCopilot project is open in Visual Studio Code.
...
1. Take a minute to compare GitHub Copilot's response with the actual project files.
    ...
    You should see a response that describes the main packages included in the project:

    - **domain** — entity classes (Book, Loan, Patron, Author, BookItem)
    - **service** — business logic (LoanService, PatronService)
    - **repository** — data access interfaces and JSON-backed implementations
    - **console** — console state-machine (ConsoleApp, ConsoleState)
...
1. Enter a prompt in the Chat view that asks how to publish your codebase to a private GitHub repository.
    ...
    ```plaintext
    @github #codebase What's the easiest way to publish my current Java project to a private GitHub repository from within Visual Studio Code?
    ```
...
1. Open the **App.java** file located in `src/main/java/com/microsoft/learning/library/` and examine the code.

1. Enter a prompt in the Chat view that generates an explanation of the App.java file.

    For example, enter the following prompt in the Chat view:

    ```plaintext
    /explain #codebase Explain the App.java file
    ```
...
1. Take a minute to review the detailed response generated by GitHub Copilot.
    ...
    You should see a response that includes an overview and a breakdown that explains how `App.java` acts as the entry point, initializes the `JsonDataStore`, constructs service and repository instances, and starts the `ConsoleApp` state machine.
...
1. Expand the `src/main/java/com/microsoft/learning/library/repository/json/` folder in the EXPLORER view.

1. Use a drag-and-drop operation to add the following files from the EXPLORER view to the Chat context: **JsonDataStore.java**, **JsonLoanRepository.java**, and **JsonPatronRepository.java**.
...
1. Take a couple minutes to read through the response.
    ...
    You should see a response that describes each of the data access classes (**JsonDataStore**, **JsonLoanRepository**, and **JsonPatronRepository**) and how they work together to manage data access in the application. Key methods, such as **ensureDataLoaded**, **saveLoans**, and **savePatrons**, should be mentioned in the response.

1. Take a minute to examine the JSON data files that are used to simulate library records.

    The JSON data files are located in the **data/Json/** folder.
...
1. To run the application, enter the following command in the terminal:
...
1. To generate project documentation for your README file, enter the following prompt:
    ...
    ```plaintext
    #codebase I need you to generate the contents of a README.md file that I can use for the current Java code repository. Use "Library App" as the project title. The README file should include the following sections: Description, Project Structure, Key Classes and Interfaces, Usage, License. Format all sections as raw markdown. Use a bullet list with indents to represent the project structure. Do not include ".gitignore" or the ".github", "target", and ".mvn" folders. I want to add the suggested content to the README.md file that's open in the editor.
    ```
...
In this exercise, you learned how to use GitHub Copilot to analyze and document a Java codebase. Using the Chat view in Ask mode, you used chat participants, chat variables, and slash commands to generate explanations for the project structure, key classes, and data access classes. You improved response relevance by adding files to the Chat context using drag-and-drop. You also ran the application to understand its behavior, and used GitHub Copilot to generate content for a README.md file.
...
- **`Failed to load JSON data`**: Run `mvn exec:java` from the project root (`AccelerateDevGHCopilot/`) so the `data/Json` path resolves correctly.
- **No tests discovered**: Ensure `mvn test` is run from the directory containing `pom.xml`.

## Trainer notes

- C# source remains in `az-2007-m2-explain-document/AccelerateDevGHCopilot`.
- Java source is in `az-2007-m2-explain-document-java/AccelerateDevGHCopilot`.
- Service behavior and status messages intentionally mirror the C# version for parity.
