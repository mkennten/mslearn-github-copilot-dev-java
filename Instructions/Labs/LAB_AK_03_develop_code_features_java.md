...
This is the Java variant of the Module 3 exercise. The original C# instructions are in `LAB_AK_03_develop_code_features.md`.
...
If you're using a local PC:

- For help enabling your GitHub Copilot subscription in Visual Studio Code, open the following link: <a href="https://go.microsoft.com/fwlink/?linkid=2320158" target="_blank">Enable GitHub Copilot within Visual Studio Code</a>.
...
You need to open the Java starter project for this module in Visual Studio Code.

Use the following steps to set up the library application:

1. Open a browser window in your lab environment.

1. To download a zip file containing the library application, paste the following URL into your browser's address bar: [GitHub Copilot lab - develop code features (Java)](https://github.com/MicrosoftLearning/mslearn-github-copilot-dev/raw/refs/heads/main/DownloadableCodeProjects/Downloads/AZ2007LabAppM3Java.zip)

    The zip file is named **AZ2007LabAppM3Java.zip**.

1. Extract the files from the **AZ2007LabAppM3Java.zip** file.

    For example:

    1. Navigate to the downloads folder in your lab environment.
    1. Right-click **AZ2007LabAppM3Java.zip**, and then select **Extract all**.
...
Use the following steps to publish the project to a private GitHub repository:

1. Open the GitHub Copilot Chat view in Visual Studio Code.

1. Set the chat mode to **Ask** and select the **Auto** model.

1. Enter the following prompt to learn how to publish to GitHub from VS Code:
    ...
    > **Prompt goal**: Generate a step-by-step plan using the `@github` participant, which consults GitHub-specific knowledge to provide accurate instructions.

1. Follow the steps suggested by GitHub Copilot to publish the project to a new private GitHub repository named **AccelerateDevGHCopilot-Java**.
...
1. Enter the following prompt to understand how the data store works:
    ...
    ```plaintext
    /explain #codebase How does JsonDataStore load and serve book data? What methods are available for querying books?
    ```

    > **Prompt goal**: Use the **/explain** slash command with **#codebase** to generate a detailed walkthrough of the data loading and access pattern. Understanding this is essential before implementing `searchBooks`.
...
1. Run the tests to verify the new tests pass:
    ...
    > ```plaintext
    > #codebase The test searchBooks_returnsEmptyListWhenNoMatch fails with: <error message>. What is wrong with the test setup?
    > ```
...
1. In the Chat view, enter the following prompt to generate a pull request description:
    ...
    ```plaintext
    #codebase Generate a pull request description for the book-search feature branch. Include: what was implemented, which files were changed, and how to test the changes. Format it as markdown.
    ```
...
In this exercise, you used GitHub Copilot to develop a new code feature in a Java library application. You published the project to a private GitHub repository, created a feature branch, and used inline chat, Ask mode, and Agent mode to implement the `searchBooks` method in `JsonBookRepository`. You evaluated the implementation for edge cases using GitHub Copilot's review capabilities, added unit tests with Mockito, and used GitHub Copilot to generate a commit message and pull request description before merging the feature branch into main.
...
- The C# source for this module is in `az-2007-m3-develop-code/AccelerateDevGHCopilot`.
- The Java starter for this module is in `az-2007-m3-develop-code-java/AccelerateDevGHCopilot`.
- The `searchBooks` method in `JsonBookRepository` starts as a `throw new UnsupportedOperationException(...)` stub that students implement.
